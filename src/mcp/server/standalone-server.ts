...
async function handleEthicalAnalysis(args: any) {
  ...
  const analysis = {
    ...
    stakeholderImpact: stakeholders?.map((s: string) => ({
      stakeholder: s,
...
async function handleTechnicalSupport(args: any) {
...
async function handleAgentDiscovery(args: any) {
...
async function handleWorkflowAnalysis(args: any) {
...
async function main() {
  try {
    ...
    console.error(`Server: ${server.serverInfo.name} v${server.serverInfo.version}`);
